  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-8 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          <Link href="/" className="text-2xl font-bold text-gray-900 hover:text-blue-600">Red CheoSys</Link>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          <Link href="/search" className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${!tipo ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>Todos</Link>
          <Link href="/search?tipo=necesita" className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tipo === 'necesita' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>🚨 Necesita</Link>
          <Link href="/search?tipo=ofrece" className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tipo === 'ofrece' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>🤝 Ofrece</Link>
          <Link href="/search?tipo=desaparecido" className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tipo === 'desaparecido' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>👤 Desaparecidos</Link>
          <Link href="/search?tipo=centro" className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tipo === 'centro' ? 'bg-blue-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-100'}`}>📍 Centros</Link>
        </div>

        <div className="mb-4 text-sm text-gray-600">
          {results.estimatedTotalHits > 0 ? (
            <p>{results.estimatedTotalHits} resultados encontrados en {results.processingTimeMs}ms</p>
          ) : (
            ...
            <article key={hit.id} className="bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition-shadow">
              ...
              {hit.imagenes && hit.imagenes.length > 0 && (
                <div className="mb-3 grid grid-cols-3 gap-2">
                  {hit.imagenes.slice(0, 3).map((img: string, idx: number) => (
